...
Repo: `yowiprod/yt-auto-health`

## Stack

- **Orchestration:** n8n — workflow JSON files under `n8n/`
...
exception when touching anything under `n8n/`:
...
- Create `n8n/sub-<n>.json` with a `When called by another workflow` trigger.
...
- `n8n/workflow-jotatiger-v2.json` — previous iteration of the main pipeline
- `n8n/workflow-jotatiger-v3.json` — current main pipeline
- `n8n/sub-overlay-pipeline.json` — video overlays subworkflow (libass title, subtitle, watermark) _[to be created]_
- _(add new subworkflows here as they are created)_

When starting work, always confirm with the user which version (v2, v3, v4...) is the
current target before editing.
...
- All overlay logic lives in `n8n/sub-overlay-pipeline.json`, not in the main workflow.
...
- Never run two Claude Code sessions writing to `n8n/` simultaneously across PCs.
